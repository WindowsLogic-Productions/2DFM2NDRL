        ...
        if (match_tok && remote_addr && *remote_addr) {
            ...
            if (colon != std::string::npos) {
                ...
                if (inet_pton(AF_INET, ip_s.c_str(), &peer_ia) == 1 &&
                    ...
                    // Same-LAN candidate: peer's private addr from the launcher
                    // (FM2K_PEER_LAN_ADDR / hub local_ip exchange). Punched
                    // alongside the reflexive addr so same-router pairs go
                    // direct over the LAN.
