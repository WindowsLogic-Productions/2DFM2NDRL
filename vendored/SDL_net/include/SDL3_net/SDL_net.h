...
/**
 * Create a TCP client stream socket bound to a specific local port before
 * connecting. Variant of NET_CreateClient added for FM2K Rollback's TCP
 * simultaneous-open NAT hole-punch — the spectator's outbound connect
 * must source from a known local port (the same port the host's punch
 * targeted) so most cone NATs accept the inbound reply SYN-ACK.
 *
 * Behaves exactly like NET_CreateClient except for two extra steps
 * inserted between socket() and connect():
 *   * setsockopt(SO_REUSEADDR) so the same local port can be in use by
 *     a server-side listener on the same process.
 *   * bind() to (any-interface, `local_port`).
 *
 * `local_port` 0 falls back to NET_CreateClient's behavior — kernel
 * picks an ephemeral source port. Useful for callers that want
 * conditional bind without two code paths.
 *
 * \param address the address of the remote server to connect to.
 * \param port the port on the remote server to connect to.
 * \param local_port the local port to bind the connecting socket to,
 *                   or 0 for kernel-ephemeral.
 ...
 * \sa NET_CreateClient
 */
extern SDL_DECLSPEC NET_StreamSocket * SDLCALL NET_CreateClientBound(NET_Address *address, Uint16 port, Uint16 local_port);
