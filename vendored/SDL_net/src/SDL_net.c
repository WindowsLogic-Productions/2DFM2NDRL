...
NET_StreamSocket *NET_CreateClientBound(NET_Address *addr, Uint16 port, Uint16 local_port)
{
    if (local_port == 0) {
        return NET_CreateClient(addr, port);
    }
    if (addr == NULL) {
        SDL_InvalidParamError("address");
        return NULL;
    ...
    // SO_REUSEADDR + bind(local_port) — TCP simultaneous-open punch lets
    // the spectator's outbound connect source from the same port their
    // TCP listener already holds, so port-restricted-cone NATs accept
    // the host's reply SYN-ACK on that same 4-tuple. Ignore bind
    // failures (rare TIME_WAIT race) — fall back to ephemeral; the
    // connect itself still proceeds and may succeed for full-cone NATs.
    {
        const int reuse = 1;
        ...
        SDL_memset(&local, 0, sizeof(local));
        local.sin_family      = AF_INET;
        local.sin_addr.s_addr = INADDR_ANY;
        local.sin_port        = htons(local_port);
        bind(sock->handle, (struct sockaddr *)&local, sizeof(local));
    }
